...
// Импорт данных из твоего нового файла
...
export default function App() {
  ...
  // Логика кнопки "Случайное слово"
  const getRandomWord = () => {
    const randomEntry = DICT[Math.floor(Math.random() * DICT.length)];
    setTopic("all");
    setAlpha("all");
    setDialect("all");
    setQuery(randomEntry.meg);
  };
  ...
  const UI = {
    ru:{title:"Мегрельский словарь",   sub:"Климов & Каджаиа, 2026",  ph:"Поиск слова…",    noR:"Ничего не найдено", tot:"слов в базе", sin:"Искать в:"},
    en:{title:"Mingrelian Dictionary", sub:"Klimov & Kadjaia, 2026",   ph:"Search a word…",  noR:"Nothing found",     tot:"words",       sin:"Search in:"},
    ge:{title:"მეგრული ლექსიკონი",    sub:"კლიმოვი & კაჯაია, 2026",  ph:"სიტყვის ძიება…", noR:"ვერ მოიძებნა",      tot:"სიტყვა",      sin:"ძებნა:"},
  };
  const t = UI[uiLang];
  const FLAG = {ru:"🇷🇺", en:"🇬🇧", ge:"🇬🇪"};
      ...
      <div style={{maxWidth:700,margin:"0 auto",padding:"16px 14px 60px"}}>
            ...
            {alphaList.map(l=>{
              ...
              return (
                <button key={l} className="pill" onClick={()=>setAlpha(l)} style={{
                  whiteSpace:"nowrap", minWidth: isAll ? "auto" : 34, padding: isAll ? "5px 11px" : "3px 5px",
                  fontSize: isAll ? 12 : 20, fontFamily:"'Noto Serif Georgian',Georgia,serif",
                  background: active ? "#7dcf7d" : "rgba(80,160,80,0.1)", color: active ? "#0f1a12" : "#b8d8b8",
                  fontWeight: active ? "bold" : "normal", border:"1px solid rgba(80,160,80,0.2)",
        ...
        <div className="fu" style={{background:"rgba(80,160,80,0.07)",border:"1px solid rgba(80,160,80,0.26)",borderRadius:16,padding:"12px 14px",marginBottom:12}}>
          <div style={{display:"flex", gap:8}}>
            ...
            <button onClick={getRandomWord} className="pill" title="Случайное слово" style={{padding:"0 12px", background:"rgba(125,207,125,0.15)", border:"1px solid rgba(125,207,125,0.3)", fontSize:20}}>
              🎲
            </button>
          </div>
                ...
                <div key={i} className="card" style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(80,160,80,0.16)",borderRadius:13,padding:"12px 14px",position:"relative"}}>
                  <div style={{position:"absolute",top:10,right:10,display:"flex",gap:3}}>
                    {hasDialects ? Object.keys(entry.dialects).map(d=>(
                      ...
                      <div style={{fontSize:9,padding:"2px 6px",borderRadius:6,fontWeight:"bold",background:entry.dialect==="sam"?"rgba(80,140,255,0.15)":"rgba(255,160,80,0.15)",color:entry.dialect==="sam"?"rgba(140,180,255,0.9)":"rgba(255,190,120,0.9)",border:"1px solid rgba(80,140,255,0.25)"}}>{entry.dialect==="sam"?"сам.":"сен."}</div>
                    )}
                  </div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"5px 10px"}}>
                    {[ {lbl:"ქართ.", val:entry.geo, col:"rgba(180,200,255,0.85)"}, {lbl:"Рус.", val:entry.ru, col:"rgba(232,224,204,0.85)"}, {lbl:"Eng.", val:entry.en, col:"rgba(200,222,200,0.85)"} ].map(({lbl,val,col})=>(
                      <div key={lbl}>
        ...
        <footer style={{textAlign:"center",marginTop:40,color:"rgba(232,224,204,0.15)",fontSize:11,lineHeight:2}}>
          ...
          <div>Мегрельско-русско-грузинский словарь. М.: 2026.</div>
        </footer>
      </div>
    </div>
  );
