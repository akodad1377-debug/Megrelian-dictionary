...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0f1a12",fontFamily:"Georgia,serif",color:"#e8e0cc",paddingBottom:40}}>
      <style>{`.sc::-webkit-scrollbar{display:none}.sc{scrollbar-width:none} input:focus{outline:none}`}</style>
      ...
      <header style={{position:"sticky",top:0,zIndex:100,background:"#0f1a12",padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center", borderBottom:"1px solid rgba(125,207,125,0.1)"}}>
        ...
        <div style={{display:"flex",gap:6}}>
          {["ru","en","ge"].map(l=>(
            <button key={l} onClick={()=>setUiLang(l)} style={{background:uiLang===l?"#7dcf7d":"#1a261c",border:"none",borderRadius:12,padding:"6px 10px",color:uiLang===l?"#0f1a12":"#e8e0cc",fontSize:11,cursor:"pointer",fontWeight:uiLang===l?"bold":"normal"}}>
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"16px 16px"}}>
        <div style={{fontSize:10,opacity:0.3,marginBottom:8,letterSpacing:1,fontWeight:"bold"}}>{t.alf}</div>
        <div className="sc" style={{display:"flex",gap:6,overflowX:"auto",marginBottom:16,paddingBottom:4}}>
          {alphaList.map(l=>(
            <button key={l} onClick={()=>setAlpha(l)} style={{
              minWidth:38,height:38,borderRadius:19,border:"none",
              background:alpha===l?"#7dcf7d":"#1a261c", color:alpha===l?"#0f1a12":"#e8e0cc",
              fontSize:l==="all"?12:18,cursor:"pointer",transition:"0.2s"
        ...
        <div style={{fontSize:10,opacity:0.3,marginBottom:8,letterSpacing:1,fontWeight:"bold"}}>{t.dial}</div>
        <div style={{display:"flex",gap:8,marginBottom:24,flexWrap:"wrap"}}>
          {[
            ...
            <button key={d.k} onClick={()=>handleGlobalDialectChange(d.k)} style={{
              padding:"8px 16px",borderRadius:18,border:"1px solid rgba(125,207,125,0.1)",
        ...
        <div style={{background:"#162218",borderRadius:16,padding:14,border:"1px solid #223324",marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:8,fontSize:11, borderTop:"1px solid rgba(125,207,125,0.05)", paddingTop:10, alignItems:"center"}}>
            <span style={{opacity:0.4}}>Искать в:</span>
            {[["all","Все"],["meg","მეგ."],["ru","Рус."],["en","Eng."],["geo","ქар."]].map(([k,v])=>(
              <button key={k} onClick={()=>setSearchIn(k)} style={{background:searchIn===k?"#7dcf7d":"#223324",color:searchIn===k?"#000":"#7dcf7d",border:"none",borderRadius:8,padding:"3px 10px",cursor:"pointer",fontWeight:searchIn===k?"bold":"normal"}}>{v}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {results.map((e,i)=>{
            ...
            return (
              <div key={i} style={{background:"#162218",borderRadius:16,padding:18,position:"relative",border:"1px solid #223324"}}>
                {hasD ? (
                  <div style={{position:"absolute",top:14,right:14,display:"flex",gap:4}}>
                    {Object.keys(e.dialects).map(dk=>(
                      <button key={dk} onClick={()=>setCardDialects(p=>({...p,[e.meg]:dk}))} style={{fontSize:9,padding:"3px 8px",borderRadius:6,border:"none",background:act===dk?"#3a5a40":"#0f1a12",color:act===dk?"#7dcf7d":"#444",fontWeight:"bold", cursor:"pointer"}}>{dk==="sam"?"сам.":"сен."}</button>
                    ))}
                  </div>
                ) : e.dialect && (
                  <div style={{position:"absolute",top:14,right:14,fontSize:9,padding:"3px 8px",background:"#0f1a12",color:"#3a5a40",borderRadius:6, fontWeight:"bold"}}>{e.dialect==="sam"?"сам.":"сен."}</div>
                )}
                <div style={{fontSize:32,fontWeight:"bold",color:"#7dcf7d",marginBottom:2,fontFamily:"'Noto Serif Georgian',serif"}}><HL text={dMeg} q={query}/></div>
                <div style={{fontSize:13,opacity:0.4,marginBottom:16,fontStyle:"italic"}}>[{dTr}]</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
                  {[{l:"ქართ.",v:e.geo},{l:"РУС.",v:e.ru},{l:"ENG.",v:e.en}].map((c,idx)=>(
                    <div key={idx}><div style={{fontSize:9,opacity:0.3,marginBottom:3,letterSpacing:0.5}}>{c.l}</div><div style={{fontSize:14,lineHeight:1.3}}><HL text={c.v} q={query}/></div></div>
                  ))}
                </div>
              </div>
            );
